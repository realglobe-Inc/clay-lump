...
function mergeSheets (target, ...sources) {
  return co(function * () {
    ...
    // Merge known records
    {
      let cursor = yield target.rawCursor()
      while (yield cursor.hasNext()) {
        let targetRecord = yield cursor.next()
        let { key } = targetRecord
        let sourceRecord = yield source.rawGet(key)
        let isNew = !!sourceRecord && (targetRecord.date <= sourceRecord.date)
        if (isNew) {
          yield target.rawSet(key, sourceRecord)
        }
      }
    }
  })
